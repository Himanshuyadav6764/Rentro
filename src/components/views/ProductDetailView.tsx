...
export default function ProductDetailView({ productId, onBack, onChatWithOwner, onRent }: ProductDetailViewProps) {
   ...
   React.useEffect(() => {
      ...
      async function hydrateItem() {
         ...
         try {
            const listing = await fetchListingById(productId);
            if (!cancelled) {
               setItem(listing);
            }
         } catch {
   ...
   const handleMessageOwner = () => {
      if (productId && item?.owner_id) {
         ...
         params.set('userId', item.owner_id);
   ...
   const handleRent = () => {
      if (productId) {
         router.push(`/rent/checkout?itemId=${encodeURIComponent(productId)}`);
         return;
      }
            ...
            <div className="relative w-full h-[55vh] md:h-[65vh] min-h-[450px]">
               ...
               <div className="absolute bottom-16 right-8 md:right-16 bg-brand text-white p-6 rounded-[2.8rem] shadow-[0_20px_60px_rgba(27,82,214,0.4)] border border-white/20 animate-in zoom-in duration-700 delay-300">
                  ...
                  <div className="flex items-baseline gap-1.5">
                     <span className="text-4xl font-black tracking-tighter">₹ {dailyPrice}</span>
                  </div>
               </div>
               ...
               <div className="absolute bottom-16 left-8 md:left-16 flex items-center gap-3 bg-white/20 backdrop-blur-xl rounded-2xl px-5 py-3 text-white border border-white/10 shadow-2xl">
                  ...
                  <span className="text-[14px] font-black uppercase tracking-[0.1em]">{locationLabel}</span>
               </div>
            </div>
            ...
            <div className="relative -mt-16 bg-[#f8faff] rounded-t-[4rem] px-6 md:px-16 pt-16 pb-60 max-w-6xl mx-auto w-full">
               ...
               <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8 mb-12">
                  <div className="flex-1">
                     ...
                     <h1 className="text-5xl md:text-6xl font-black text-slate-900 tracking-tight leading-[0.9]">{title}</h1>
                  </div>
               ...
               <div className="mb-12">
                  ...
                  <div className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm leading-relaxed text-slate-600 font-medium">
                     <p className="mb-4">
                        {item?.description || (
                           <>
                              Experience extreme performance with the <span className="text-slate-900 font-bold">{title}</span>. This powerhouse is equipped for coding, editing, and heavy productivity workflows.
                           </>
                        )}
                     </p>
                     ...
                     <div className="bg-white rounded-[3rem] p-4 border border-slate-100 shadow-xl shadow-slate-200/40">
                        <div className="bg-[#f0f4f8] rounded-[2.5rem] p-8">
                           ...
                           <div className="space-y-4">
                              <PriceRow label="Daily Rental" value={`₹ ${dailyPrice}`} />
                              <PriceRow label="Security Deposit" value={`₹ ${deposit}`} isBold />
            ...
            <div className="w-full max-w-5xl bg-white/70 backdrop-blur-3xl border border-white/60 p-5 md:p-7 rounded-[3.5rem] shadow-[0_30px_100px_rgba(0,0,0,0.15)] flex items-center justify-between pointer-events-auto">
               <div className="hidden md:flex flex-col pl-4">
                  ...
                  <div className="flex items-baseline gap-2">
                     <span className="text-4xl font-black text-slate-900 tracking-tighter">₹ {totalEstimate}</span>
                     <span className="text-[15px] font-black text-slate-400 leading-none">/ 7 DAYS</span>
                  </div>
               </div>

               {isLoadingItem && (
                  <div className="hidden md:flex items-center text-xs font-black uppercase tracking-widest text-slate-400">
                     Syncing item data...
                  </div>
               )}
               ...
               <div className="flex flex-1 md:flex-none items-center gap-5 pr-2">
                  <button
                     onClick={handleRent}
                     className="flex-1 md:w-[320px] min-h-[80px] rounded-[2.2rem] font-black text-xl tracking-tight shadow-[0_20px_50px_rgba(27,82,214,0.4)] hover:scale-[1.03] active:scale-[0.97] transition-all flex items-center justify-center gap-4 group px-10 bg-brand text-white"
                  >
                     RENT NOW
